...
import com.solexgames.robot.commons.annotation.RegisteredCommand;
import com.solexgames.robot.commons.impl.PublicCommonsUser;
...
public class SyncCommand {
    ...
    public void onSyncCommand(
        ...
        if (networkPlayer == null) {
            publicCommonsUser.reply(EmbedUtil.getEmbed(publicCommonsUser.getUser(), RobotPlugin.getInstance().getLangMap().get("invalid-code|title"), RobotPlugin.getInstance().getLangMap().get("invalid-code|description"), java.awt.Color.RED),
        ...
        if (networkPlayer.isSynced()) {
            publicCommonsUser.reply(EmbedUtil.getEmbed(publicCommonsUser.getUser(), RobotPlugin.getInstance().getLangMap().get("already-synced|title"), RobotPlugin.getInstance().getLangMap().get("already-synced|description"), java.awt.Color.RED),
                    message -> message.delete().queueAfter(5L, TimeUnit.SECONDS)
            );
            return;
        }

        final Role role = publicCommonsUser.getTextChannel().getGuild().getRolesByName(RobotPlugin.getInstance().getLangMap().get("syncing|role"), false).get(0);

        if (role == null) {
            publicCommonsUser.reply("Something went wrong while trying to execute that command. Please contact a server admin asap.");
            publicCommonsUser.reply("Result: **\"The role with the name " + RobotPlugin.getInstance().getLangMap().get("syncing|role") + " does not exist.\"**");
            return;
        }

        publicCommonsUser.getMessage().delete().queue();
        publicCommonsUser.reply(EmbedUtil.getEmbed(publicCommonsUser.getUser(), RobotPlugin.getInstance().getLangMap().get("successful-sync|title"), RobotPlugin.getInstance().getLangMap().get("successful-sync|description").replace("<playerName>", networkPlayer.getName()), java.awt.Color.GREEN),
                message -> message.delete().queueAfter(5L, TimeUnit.SECONDS)
        );
        ...
        if (rankName.equalsIgnoreCase(RobotPlugin.getInstance().getLangMap().get("settings|default")) || prefix == null) {
            publicCommonsUser.getMember().modifyNickname(RobotPlugin.getInstance().getLangMap().get("syncing|format").replace("<playerName>", networkPlayer.getName())).queue();
        } else {
